            ...
            <div class="card-body">
                ...
                foreach ($key['channels'] as $channel) {
                    ...
                    ?><?= Html::beginForm('/subscriptions/default/change', 'post'); ?>
                        ...
                        <p><button type="button" class="btn btn-<?= (($channel['active']) ? 'light unsubscribe' : 'primary subscribe') ?> "><?= (($iconClass) ? '<i class="' . $iconClass . '"></i>&nbsp;' : '') ?><?= $channel['name'] ?>, <?= (($channel['active']) ? 'отписаться от рассылок' : 'подписаться на рассылки') ?></button></p>
                    <?= Html::endForm(); ?><?
                }
            ?></div>
            </div><?
